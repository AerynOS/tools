...
impl Drafter {
    ...
    pub fn run(&self) -> Result<Draft, Error> {
        ...
        let licence_matches = match_licences(&extract_root, licences_dir).unwrap_or_default();

        let licence_strings: Vec<String> = licence_matches
            .iter()
            .map(|m| {
                println!(
                ...
                m.spdx_identifier.to_owned()
            })
            .collect();

        let licenses = format_licenses(licence_strings);
